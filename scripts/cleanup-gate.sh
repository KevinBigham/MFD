...
echo "SAVE_VERSION: $save_version (expected current: 30)"
...
if [ "$save_version" != "30" ]; then
  echo "FAIL: SAVE_VERSION drifted (expected 30, got $save_version). Halt and escalate to Kevin." >&2
  exit 1
fi
