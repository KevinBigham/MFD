...
echo "SAVE_VERSION: $save_version (expected current: 38)"
...
if [ "$save_version" != "38" ]; then
  echo "FAIL: SAVE_VERSION drifted (expected 38, got $save_version). Halt and escalate to Kevin." >&2
  exit 1
fi
