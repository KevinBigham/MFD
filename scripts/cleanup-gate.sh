...
echo "SAVE_VERSION: $save_version (expected current: 29)"
...
if [ "$save_version" != "29" ]; then
  echo "FAIL: SAVE_VERSION drifted (expected 29, got $save_version). Halt and escalate to Kevin." >&2
  exit 1
fi
