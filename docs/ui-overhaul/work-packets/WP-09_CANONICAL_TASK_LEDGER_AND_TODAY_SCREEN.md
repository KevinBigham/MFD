# WP-09 — Canonical Task Ledger and Today Screen
...
| Dependencies | WP-01–WP-06; defines contract for WP-08 |
| Owner gate | None beyond packet dependencies |
| Migration rule | Reversible, compatibility-first, no production route removal |

## User problem solved

Briefing, Action Center, Inbox, badges, Chip, and Advance independently compete to define what matters.

## Objective

Create a pure derived Task Ledger/presenter and replace Monday Briefing in new mode with the bounded Today archetype.

## Scope

- Derive one immutable `TaskLedger` from current game state, lifecycle, blockers, warnings, inbox/events, and existing required-task logic.
- Define task identity, priority, urgency, phase, destination, completion, explanation, consequence, and optionality.
- Build the bounded Today screen with first-viewport context, primary task, concise required/optional groups, opponent/phase context, and readiness entry.
- Reuse the ledger for navigation badges, Chip, and Week Advance.

## Explicit non-scope

- No mutation inside selectors/presenters.
- No removal of Inbox/Action history; absorb them as sources or durable views.
- No giant customizable dashboard in the first slice.

## Exact files to add

- `apps/web/src/ui/presenters/task-ledger.ts`
- `apps/web/src/ui/presenters/today-presenter.ts`
- `apps/web/src/ui/presenters/task-ledger.test.ts`
- `apps/web/src/ui/screens/today/TodayScreen.tsx`
- `apps/web/src/ui/screens/today/TaskRow.tsx`
- `apps/web/src/ui/screens/today/TodayScreen.module.css`
- `apps/web/src/ui/screens/today/TodayScreen.test.tsx`

## Exact files to modify

- `apps/web/src/app/store/selectors.ts`
- `apps/web/src/features/monday-briefing/MondayBriefing.tsx`
- `apps/web/src/app/navBadges.ts`
...
- Pure selectors produce normalized task candidates.
- `selectTaskLedger` dedupes by semantic task key, resolves priority, and emits stable destinations.
- `presentToday` builds a view model independent of JSX.
- `TodayScreen` owns no simulation logic and renders bounded sections with progressive disclosure.
- Task completion returns to the task row and announces updated readiness.
...
- 390×844 first viewport shows season/week/phase, opponent/context, primary required task, readiness state, and next action.
- Default Today content targets no more than 2.5 phone viewports before optional history/customization.
- Desktop uses a primary task column plus compact context rail; it does not stack every widget.
- Sticky action appears only when context requires it.
...
- Phase fixtures
- task dedupe/priority/completion
- 390×844 first viewport
- ≤2.5 viewports default
- return focus
- no mutations
- legacy parity for blockers.
...
- Run every lifecycle fixture and compare legacy blockers/tasks.
- Create duplicate source signals and verify one task.
- Complete a depth/game-plan/inbox task and return with focus.
- Advance phase and verify stale tasks disappear.
- Inspect first viewport at all phone sizes.

## Required before / after screenshots

- Today with blocked task.
- Today ready state.
- Optional task expansion.
- Phone/desktop first viewport.
- Return-to-task completed state.
...
- Incorrect dedupe could hide a real blocker.
- Presenter can become another monolith if source adapters are not separated.
- Task IDs must remain stable without entering save schema.

## Rollback path

Legacy MondayBriefing remains mounted in legacy mode; presenter additions are pure.
...
- feat(tasks): add canonical pure task ledger
- feat(today): add bounded phase-aware Today screen
- feat(tasks): connect badges and return behavior
- test(tasks): prove source parity priority and lifecycle states
...
One task source drives Today/nav/Chip/readiness; first viewport contract passes; no feature data deleted.
