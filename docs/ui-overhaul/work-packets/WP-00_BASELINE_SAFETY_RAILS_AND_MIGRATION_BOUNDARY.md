...
A shell rewrite cannot begin safely while clean build/test, visual, route, save, and bundle baselines are incomplete or environment-blocked.

## Objective

Re-establish a correctly provisioned baseline; create UI-only migration control, deterministic fixture capture, route-coverage check, and progress ledger before production UI changes.

## Scope

- Install dependencies on a supported host and record Node, pnpm, browser, OS, branch, commit, and dirty state.
- Run every existing quality gate before UI code changes; preserve full logs rather than summaries alone.
- Create deterministic representative fixtures for new game, regular season, deadline, playoffs, offseason, draft, free agency, and multi-season legacy states.
- Add a UI-only migration selector that defaults to the legacy shell and cannot alter save or simulation state.
- Add a registry parity gate that proves every canonical route resolves to exactly one future surface and compatibility behavior.
- Create the implementation progress ledger with packet status, commit, checks, screenshots, rollback note, and unresolved protected-contract conflicts.
...
- No route retirement.
- No save migration.
- No repair of unrelated pre-existing failures unless separately authorized.

## Exact files to add

- `apps/web/src/ui/migration/ui-overhaul-mode.ts`
- `apps/web/src/ui/test/fixtures/ui-overhaul-fixtures.ts`
- `apps/web/e2e/ui-overhaul-baseline.spec.ts`
- `scripts/check-ui-route-coverage.mjs`
- `docs/ui-overhaul/implementation/PROGRESS_LEDGER.md`

## Exact files to modify

- `package.json`
- `apps/web/package.json`
- `apps/web/src/app/store/ui-store.ts`
...
- `UiMigrationHost` reads a UI-only preference and chooses `LegacyAppShell` or the new shell.
- `uiOverhaulFixtures` exposes deterministic setup helpers; fixtures must not become production seed logic.
- `checkUiRouteCoverage` imports the canonical registry and the UI adapter, then fails on missing, duplicate, circular, or unresolvable entries.
...
- Capture the full viewport matrix from `09_QA_ACCEPTANCE_AND_TRACEABILITY.md`.
- Store geometry measurements beside screenshots so later packets compare numbers, not impressions.
...
- Clean install
- all current typechecks/tests/build/bundle/smoke/release gates
- save fixture hash
- 79-route resolution
- baseline screenshots/geometry.
...
- Start from a clean clone or archive extraction.
- Run the command matrix and paste exact outputs into the ledger.
- Launch each deterministic state in legacy mode and capture the required baseline.
- Toggle new-shell mode; verify no game or save state changes.
- Open all 79 canonical paths and verify the route coverage script output.

## Required before / after screenshots

- Legacy baseline at phone, landscape phone, tablet, 1280×720, and 1440×900.
- Migration-mode indicator in legacy and new modes.
- Representative save fixture identity/hash evidence.
...
- A stale `node_modules` tree can create false product failures.
- Fixture setup that calls production mutations in the wrong order can consume RNG.
- A persisted migration flag could unexpectedly switch real users without a rollout policy.

## Rollback path

Remove UI-only flag/harness additions; no simulation or save data changes.
...
- test(ui): establish clean baseline and deterministic fixtures
- feat(ui-migration): add legacy-default migration boundary
- test(routes): enforce 79-route surface parity
- docs(ui-overhaul): start implementation progress ledger
...
Baseline ledger records command outputs; route count is 79/79; migration mode defaults to legacy; no save schema changes.
