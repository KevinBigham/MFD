# WP-15 — Office Finance, Contracts, Ownership, and Operations
...
Contracts, Cap Lab, Front Office, owner, facilities/operations, and governance signals lack one finance/ownership question model.

## Objective

Create Office hub Finance/Operations/Ownership sections with summary-first data and safe scenario/contract workflows.

## Scope

- Create Office hub with Overview, Finance, Contracts, Operations, Ownership, and Personnel entry points.
- Unify cap/contracts/front-office questions with summary-first consequences.
- Provide UI-only cap/scenario workbench until explicit commit.
- Preserve owner, facility, operations, governance, and contract actions.

## Explicit non-scope

- No cap math or contract rule change.
- No speculative state mutation before confirm.
- No loss of advanced finance detail.

## Exact files to add

- `apps/web/src/ui/screens/office/OfficeHub.tsx`
- `apps/web/src/ui/screens/office/FinanceSection.tsx`
- `apps/web/src/ui/screens/office/ContractsScreen.tsx`
- `apps/web/src/ui/screens/office/CapWorkbench.tsx`
- `apps/web/src/ui/screens/office/OwnershipSection.tsx`
...
- Finance presenter exposes current room, future commitments, expirations, risks, and actionable exceptions.
- `CapWorkbench` keeps scenario edits in isolated UI draft state.
- `ContractsScreen` uses priority groups and player entity links.
- Ownership/Operations sections translate signals into questions, consequences, and existing actions.
...
- Phone: summary strip, urgent exceptions, grouped contracts, filters in sheet, staged action.
- Tablet: optional contract/player pane.
- Desktop: workbench/table with frozen identity and prioritized columns.
- Scenario consequences remain visible near commit action.
...
- Healthy/over-cap/expirations/owner warning
- scenario cancel/no mutation
- contract actions
- mobile priorities
- old-path aliases.
...
- Test healthy, near-cap, over-cap, many expirations, owner warning, facility/operation decision.
- Create/cancel/commit contract and cap scenarios.
- Compare output/actions with legacy routes.
- Open old paths and verify canonical section.

## Required before / after screenshots

- Office overview.
- Finance summary.
- Contracts phone/desktop.
- Cap scenario.
- Ownership warning.
...
- Scenario drafts can leak into production store.
- Summary metrics can disagree with authoritative cap math.
- Grouping routes may hide rare governance/operations actions.

## Rollback path

Legacy finance routes remain wrappers; scenarios remain UI-only until commit.
...
- feat(office): add Office hub and finance presenter
- feat(contracts): add responsive contracts surface
- feat(cap): add isolated scenario workbench
- feat(office): migrate ownership and operations
- test(office): verify finance/action parity
...
Finance questions have canonical surfaces; every current action and cap consequence preserved.
