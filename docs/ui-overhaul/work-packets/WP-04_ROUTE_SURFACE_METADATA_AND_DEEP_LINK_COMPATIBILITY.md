...
The 79-route registry describes destinations but not future hubs, surface types, canonical paths, local tabs, return behavior, or compatibility.

## Objective

Create a UI adapter over the canonical engine registry that accounts for 79/79 paths and resolves old links into the new IA without changing engine route contracts.

## Scope

- Model the future hub, surface type, canonical destination, local section, lifecycle phase, frequency, and compatibility behavior for 79/79 routes.
- Preserve path/query/entity context through aliases and wrappers.
- Add navigation-origin metadata for exact return-to-task behavior.
- Expose badge and unlock metadata through the adapter rather than duplicating registry logic.

## Explicit non-scope

- Do not edit simulation routing contracts in the engine.
- Do not delete old URLs.
- Do not create a second manually maintained route list without parity enforcement.

## Exact files to add

- `apps/web/src/ui/routes/route-surface-types.ts`
- `apps/web/src/ui/routes/route-surface-map.ts`
- `apps/web/src/ui/routes/route-compatibility.ts`
- `apps/web/src/ui/routes/navigation-origin.ts`
- `apps/web/src/ui/routes/route-surface-map.test.ts`

## Exact files to modify

- `apps/web/src/app/currentAppRoute.ts`
...
- `ROUTE_SURFACE_MAP` is keyed by canonical route IDs/paths from the engine registry.
- `resolveCompatibleRoute` returns hub, local tab/detail, preserved params, compatibility mode, and canonical URL.
- `NavigationOrigin` stores source route, source task, entity, filter/sort state, scroll anchor, and focus target when safe.
- Unknown or locked destinations have explicit recoverable behavior.
...
- Surface metadata is viewport-neutral; shell/navigation decide presentation.
- Local tabs may become sections or sheets, but canonical destination identity remains stable.
...
- 79/79 registry parity
- old path resolution
- unlock/phase fixtures
- entity/query preservation
- return origin
...
- Run the parity script.
- Paste every old URL into a clean session.
- Verify query/hash/entity IDs survive redirects.
- Enter a detail from Today, Roster, Trade Desk, and League; return exactly.
- Test locked/offseason-only paths.

## Required before / after screenshots

- Route coverage report.
- Old URL resolving into a hub/tab.
- Return-to-task control with origin label.
...
- Redirect loops or lost query state.
- Coupling UI route IDs to mutable labels.
- Silent capability loss hidden behind a broad hub.

## Rollback path

Adapter can be bypassed; canonical registry remains untouched.
...
- feat(ui-routes): define route surface model
- feat(ui-routes): add deep-link compatibility and origin restoration
- test(ui-routes): enforce 79-route parity
...
Coverage script fails any missing/duplicate route; old paths remain functional; no route is deleted.
