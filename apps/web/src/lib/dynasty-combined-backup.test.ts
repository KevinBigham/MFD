...
describe('dynasty-combined-backup', () => {
  ...
  it('accepts older combined backups whose sidecar archive did not carry rivalries', () => {
    const json = exportDynastyCombinedBackupJson(makeCartridgeJson(), makeSidecarPayload());
    const envelope = JSON.parse(json) as { sidecars: DynastySidecarArchivePayload };
    const { rivalries: _rivalries, ...legacySidecars } = envelope.sidecars.sidecars;
    envelope.sidecars = {
      ...envelope.sidecars,
      sidecars: legacySidecars,
    };

    const result = parseDynastyCombinedBackupJson(JSON.stringify(envelope));
    ...
    expect(result.sidecarPayload.sidecars.rivalries).toBeUndefined();
  });
});
