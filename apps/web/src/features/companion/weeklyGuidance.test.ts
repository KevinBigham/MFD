...
describe('Chip weekly guidance', () => {
  ...
  it('adds deterministic dynasty-memory callbacks from read-only history layers', () => {
    const guidance = buildWeeklyGuidanceFromGame({
      week: 9,
      year: 2032,
      phase: 'regular_season',
      weekSummaries: [{ result: 'win', record: '6-3', opponentName: 'Rival Squad' }],
      teams: {
        user: {
          id: 'user',
          isUser: true,
      ...
      dynastyTimeline: [{
        ...
        week: 18,
        type: 'milestone',
        headline: 'Chicago ended the drought',
        importance: 'major',
        teamIds: ['user'],
      }],
      storylineThreads: [{
        id: 'thread-1',
        title: 'Division crown pressure',
        status: 'active',
        teamIds: ['user'],
        weeksActive: 4,
      }],
      ...
      franchiseHistory: [{
        teamId: 'user',
        year: 2031,
        playoffFinish: 'conference_loss',
      }],
    }, 0);

    expect(guidance.memoryCallbacks[0]).toContain('Chicago ended the drought');
    expect(guidance.memoryCallbacks).toContain('Division crown pressure has been building for 4 weeks; Chip has the thread pinned.');
    expect(guidance.memoryCallbacks.some((line) => line.includes('Rival Squad rivalry'))).toBe(true);
    expect(formatChipWeeklyGuidanceText(guidance)).toContain('Chicago ended the drought');
  });
