...
const PHILOSOPHY_COPY: Record<DepthChartPhilosophy, { label: string; desc: string }> = {
  best_players: {
    label: 'Best Players',
    desc: 'Start the most reliable Week 1 lineup now, even if it slows a young player’s development snaps.',
  },
  veterans_first: {
    label: 'Veterans First',
    desc: 'Start experienced players first; young backups lose early development snaps while veterans install the Week 1 calls.',
  },
  youth_bet: {
    label: 'Young Starters',
    desc: 'Start young players early for development snaps, but accept more Week 1 mistakes.',
  },
};
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
      <div style={{ ...monoSm, color: '#bbb', lineHeight: 1.6 }}>
        Decide how the Week 1 lineup gets built. Your philosophy changes which players win the last few starting jobs.
      </div>
                  ...
                  <div key={player.playerId} style={{
                    ...
                    {typeof player.fitScore === 'number' && (
                      <span style={{ ...monoSm, color: player.fitScore >= 70 ? 'var(--mfd-cyan)' : 'var(--mfd-gold)' }}>
                        Role match {Math.round(player.fitScore)}
                      </span>
                    )}
                  </div>
                );
