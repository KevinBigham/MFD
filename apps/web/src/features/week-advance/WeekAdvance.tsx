...
import {
  ...
  selectOffseasonCalendar, selectDraftRecaps, selectLeagueRivalries, selectPowerRankings,
...
export function WeekAdvance() {
  ...
  const radar = useMemo(() => {
    ...
    const majorUserInjuries = roster.filter((player) =>
      ...
      && player.injury
      && (
        player.injury.severityTier === 'severe'
  ...
  const simAheadControls: Array<{ label: string; target: SimAheadTarget }> = [
    ...
    { label: 'End Phase', target: 'end_phase' },
              ...
              {radar?.majorUserInjuries.length ? (
                <div style={{ display: 'flex', gap: '6px', flexWrap: 'wrap' }}>
                  {radar.majorUserInjuries.slice(0, 3).map((player) => (
                    <PixelBadge key={player.id} variant="red">
                      {player.pos} {player.lastName}
                    </PixelBadge>
                  ))}
                </div>
              ) : null}
