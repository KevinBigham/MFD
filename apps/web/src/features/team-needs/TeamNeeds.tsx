...
import { buildTeamWindowInput, computeTeamWindow, type TeamWindow, type TeamWindowPhase } from '../../lib/team-window';
...
function strategyLedgerDetail(strategy: string): string {
  ...
  if (strategy === 'rebuild') return 'The front office is protecting picks, youth, and cap space before splash moves.';
    ...
    <PixelPanel title="CPU Intent" accent="gold">
      <div style={autoGrid(180)}>
        ...
        {teamWindow ? (
          <PixelMetricCard
            label="Window"
            value={teamWindowLabel(teamWindow.phase)}
            accent={teamWindowAccent(teamWindow.phase)}
            detail={`${teamWindow.confidence} read // ${teamWindow.drivers.length} drivers`}
          />
        ) : null}
      </div>
      {teamWindow ? (
        <div style={{ display: 'flex', flexDirection: 'column', gap: '8px', marginTop: '12px' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '8px', flexWrap: 'wrap' }}>
            <span style={{ ...pixelSm, color: 'var(--mfd-text-faint)' }}>COMPETITIVE WINDOW</span>
            <PixelBadge variant={teamWindowAccent(teamWindow.phase)}>{teamWindowLabel(teamWindow.phase)}</PixelBadge>
          ...
          {teamWindow.drivers.map((driver) => (
            ...
            >
              <PixelBadge variant={teamWindowAccent(teamWindow.phase)}>{driver.label}</PixelBadge>
              <span style={{ ...monoSm, color: 'var(--mfd-text-dim)', lineHeight: 1.5 }}>{driver.detail}</span>
            </div>
          ))}
          <div style={{ ...monoSm, color: 'var(--mfd-text-faint)', lineHeight: 1.6 }}>
            Sources: {teamWindow.sourceRefs.join(' / ')}. This read model explains the selected CPU club; it does not change offers, bids, draft logic, or CPU behavior.
          </div>
        </div>
      ) : null}
      <div style={{ display: 'flex', flexDirection: 'column', gap: '8px', marginTop: '12px' }}>
      ...
      <div style={{ ...monoSm, color: 'var(--mfd-text-dim)', lineHeight: 1.6, marginTop: '10px' }}>
        The booth is reading club posture, cap space, roster holes, and public trade-board smoke.
...
export function TeamNeeds() {
  ...
  const teamWindow = useMemo(() => (compareTeam ? computeTeamWindow(buildTeamWindowInput(compareTeam, {
    currentYear: game?.year ?? null,
    teamNeeds: compareReport,
    franchiseHistory: game?.franchiseHistory ?? [],
  })) : null), [compareTeam, compareReport, game?.franchiseHistory, game?.year]);
  ...
  return (
    <div style={screenStackStyle}>
      ...
      <CpuIntentPanel compareTeam={compareTeam} report={compareReport} strategyHistory={strategyHistory} teamWindow={teamWindow} />
