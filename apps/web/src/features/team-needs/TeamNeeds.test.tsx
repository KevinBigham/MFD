...
const mockState = {
  game: {
    year: 2028,
    franchiseHistory: [
      { teamId: 'rival', year: 2027, wins: 11, losses: 6, ties: 0, playoffFinish: 'division' },
    ],
  },
  ...
  teams: {
    ...
    rival: {
      ...
      ties: 0,
      draftPicks: [
        { round: 1, pick: 22, originalTeamId: 'rival', currentTeamId: 'rival', year: 2029, isCompPick: false },
        { round: 2, pick: 54, originalTeamId: 'rival', currentTeamId: 'rival', year: 2029, isCompPick: false },
        { round: 3, pick: 86, originalTeamId: 'rival', currentTeamId: 'rival', year: 2030, isCompPick: false },
      ],
      roster: [
        { id: 'rival-qb', name: 'Cole Hart', firstName: 'Cole', lastName: 'Hart', pos: 'QB', ovr: 91, age: 29, isStarter: true, tradeBlock: false },
        { id: 'rival-vet', name: 'Austin Vet', firstName: 'Austin', lastName: 'Vet', pos: 'WR', ovr: 83, age: 31, isStarter: true, tradeBlock: true },
        { id: 'rival-core', name: 'Austin Core', firstName: 'Austin', lastName: 'Core', pos: 'DL', ovr: 87, age: 25, isStarter: true, tradeBlock: false },
        { id: 'rival-ol', name: 'Austin Wall', firstName: 'Austin', lastName: 'Wall', pos: 'OL', ovr: 80, age: 28, isStarter: true, tradeBlock: false },
      ],
    },
  },
  report: {
...
describe('TeamNeeds', () => {
  ...
  it('renders selected CPU intent from saved strategy and needs sources without mutating AI state', () => {
    ...
    expect(markup).toContain('CONTEND');
    expect(markup).toContain('clear read // 4 drivers');
    expect(markup).toContain('Contender spine');
    expect(markup).toContain('QB 91 OVR, core 85 OVR, 1 starters 30+');
    expect(markup).toContain('team:rival:roster');
    expect(markup).toContain('does not change offers, bids, draft logic, or CPU behavior');
    ...
    expect(markup).toContain('club posture, cap space, roster holes, and public trade-board smoke');
