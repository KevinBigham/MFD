 ...
 *
 * Defines all 11 positions with rating attributes, weights,
 * side (O/D/K), and category groupings.
 */
...
export interface PositionDef {
  ...
  w: readonly number[];
  side: PositionSide;
  cat: Record<string, readonly string[]>;
}

export const POS_DEF: Record<Position, PositionDef> = {
  QB: {
    r: [
...
export const ALL_POSITIONS: Position[] = Object.keys(POS_DEF) as Position[];
export const OFF_POSITIONS: Position[] = ALL_POSITIONS.filter(p => POS_DEF[p].side === 'O');
export const DEF_POSITIONS: Position[] = ALL_POSITIONS.filter(p => POS_DEF[p].side === 'D');
export const SPEC_POSITIONS: Position[] = ALL_POSITIONS.filter(p => POS_DEF[p].side === 'K');
